...
void do_shit(int filedescr1,int filedescr2)
{
	...
	while(1)
	{
		...
		while(1)
		{
			...
			if(cod_produs1 == cod_produs2)
			{
				...
				if(cantitate_noua > 0)
				{
					...
					printf("Notificare: [PID: %d] a actualizat produsul [%d]: %d -> %d\n",getpid(),cod_produs2,cantitate2,cantitate_noua);
				}
				else if(-1 == lseek(filedescr1, sizeof(int), SEEK_CUR))
				{
					perror("Eroare (4) la repozitionarea inapoi in fisier"); exit(18);
				}
		...
		if(cantitate1 > 0 && gasit == 0)
		{
			if(-1 == write(filedescr1, &cod_produs1, sizeof(int)))
			{
				perror("Eroare la adaugarea unui nou produs!");
				exit(17);
			}
			...
			printf("Notificare: [PID: %d] a adaugat un produs nou: [%d] cu cantitatea [%d]\n",getpid(),cod_produs1,cantitate1);
		}
		else if(cantitate1 < 0 && gasit == 0)
		{
			printf("Eroare [PID: %d]:Nu a fost gasit produsul [%d] iar cantitatea este [%d]\n",getpid(),cod_produs1,cantitate1);
			exit(20);
		}
    }
